...
public class ItemController {
    ...
    public void createPage(Long id) {
        ...
        categoryList.add(categoryService.findById(spu.getCid1()).getName());//一级分类
        categoryList.add(categoryService.findById(spu.getCid2()).getName());//二级分类
        categoryList.add(categoryService.findById(spu.getCid3()).getName());//三级分类

        //生成SKU地址列表
        Map urlMap = new HashMap();
        for (Sku sku : skuList) {
            //对规格json字符串进行排序
            String specJson = JSON.toJSONString(JSON.parseObject(sku.getSpecs()), SerializerFeature.SortField.MapSortField);
        ...
        for (Sku sku : skuList) {
            ...
            Map paraItems = JSON.parseObject(spu.getParams());//SPU参数列表
            dataModel.put("paraItems", paraItems);
            Map specItems = JSON.parseObject(sku.getSpecs());//当前SKU规格
            dataModel.put("specItems", specItems);

            //规格选择面板
            Map<String, List> specMap = (Map) JSON.parse(spu.getParams());
            for (String key : specMap.keySet()) {//循环规格名称
                ...
                for (String value : list) {//循环规格选项值
                    ...
                    //商品详细页地址
                    Map spec = JSON.parseObject(sku.getSpecs());//当前SKU规格
                    spec.put(key, value);
